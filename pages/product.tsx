...
class Product extends React.Component<IProductProps, IProductState> {
    ...
    constructor(props: IProductProps) {
        ...
        this.state = {
            email: "",
            discountCode: "",
            discountMsg: defaultDiscountMsg,
        };
    ...
    componentDidUpdate() {
        ...
        if (this.state.email === "" && this.props.query.email) {
            this.setState({ email: this.props.query.email as string });
        }
    ...
    private onDiscountCodeUpdated() {
        if (this.state.discountCode === "") {
            this.setState({ discountMsg: defaultDiscountMsg });
            return;
        }

        const url = baseApiUrl + "/discount-code?code=" + this.state.discountCode;
        axios.get(url)
            .then(response => {
                if (response.data.ok) {
                    this.setState({ 
                        discountMsg: `You have a discount of ${response.data.discount.toString()}% which means you'll pay ${response.data.payment.toString()} USD.`,
                    });
                }
                else {
                    this.setState({ 
                        discountMsg: `Your discount code isn't valid.`,
                    });
                }
            })
            .catch(err => {
                console.error(`An error occurred retreiving querying server for discount code ${this.state.discountCode}.`);
    ...
    //
    private async onPurchaseWithPayPal() {
        try {
            let purchaseEmail = this.state.email;
            if (purchaseEmail) {
    ...
    //
    private async onPurchaseWithStripe() {
        try {
                ...
                <div className="product">
                    ...
                    <div className="mt-16">
                        <a 
                            ...
                            onClick={this.onPurchaseWithPayPal} 
                            >
                            <div 
                    ...
                    <div className="mt-8">
                        <button 
                            id="checkout-button"
                            onClick={this.onPurchaseWithStripe}
                            >
